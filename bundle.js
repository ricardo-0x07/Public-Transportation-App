...
/* global window, $, google, document, ko, Backbone, Handlebars, navigator */
/* eslint no-negated-condition: 2*/

// Namespace our app
/* eslint-disable no-use-before-define*/
var app = app || {};
console.log('in zalls');
...
app.EventCollection = Backbone.Collection.extend({
  url: '/allEvents',
  localStorage: new Backbone.LocalStorage('app.EventCollection'),
  model: app.Event
});
...
app.UserCollection = Backbone.Collection.extend({
  url: '/allUsers',
  localStorage: new Backbone.LocalStorage('app.UserCollection'),
  model: app.User
});
...
app.ContactCollection = Backbone.Collection.extend({
  url: '/allContacts',
  localStorage: new Backbone.LocalStorage('app.ContactCollection'),
  model: app.Contact
});
...
app.EventsListView = Backbone.View.extend({
  ...
  initialize: function() {
    this.listenTo(app.Events, 'change', this.render);
    this.listenTo(app.Events, 'add', this.addOne);
    this.listenTo(app.Events, 'reset', this.render);
  },
  ...
  addOne: function(event) {
    var eventSummaryView = new app.EventSummaryView({model: event});
    $('.event_list').prepend(eventSummaryView.render());
  },
  // Add all items in the **event** collection at once.
  addAll: function() {
    this.$list.html('');
    app.Events.each(this.addOne, this);
  },
  filterList: function(filter) {
    var _this = this;
    app.Events.forEach(function(model) {
...
app.EventSummaryView = Backbone.View.extend({
  tagName: 'a',
  ...
  onClick: function() {
    app.sortRouter.navigate('events/' + this.model.id, {trigger: true});
  },
  ...
  render: function() {
    ...
    $(this.el).click(_this.model, function() {
      ...
      app.sortRouter.navigate('events/' + _this.model.id, {trigger: true});
    });
...
app.EventView = Backbone.View.extend({
  ...
  loadScripts: function() {
    ...
    var scripts = [
		{src: 'https://maps.googleapis.com/maps/api/js?key=AIzaSyBeDnT9G9IMIFxH9jjTUxmzSKwfForVk04&libraries=places&callback=app.eventView.initAutocomplete',
		attrs: ['async', 'defer']
		}];
    ...
    this.model.destroy({
      success: function() {
        ...
        if (app.Events.length > 0) {
          var last = app.Events.length - 1;
          app.sortRouter.navigate('events/' + app.Events.at(last).id, {trigger: true});
        } else {
          app.sortRouter.navigate('events/new', {trigger: true});
        }
      }
    });
    return false;
...
app.ContactView = Backbone.View.extend({
  tagName: 'article',
  className: 'contact',
  template: Handlebars.compile($('#tpl_contact').html()),
...
app.SignUpView = Backbone.View.extend({
  ...
  loadScripts: function() {
    ...
    var scripts = [
			{src: 'https://maps.googleapis.com/maps/api/js?key=AIzaSyBeDnT9G9IMIFxH9jjTUxmzSKwfForVk04&signed_in=true&libraries=places&callback=app.signUpView.initAutocomplete',
			attrs: ['async', 'defer']
			}];
    scripts.forEach(function(s) {
      var script = document.createElement('script');
...
app.SignInView = Backbone.View.extend({
  ...
  submit: function() {
    ...
    this.model = app.Users.findWhere(formValues);
...
app.Router = Backbone.Router.extend({
  ...
  home: function() {
    console.log('Home');
    $('.shell').html(new app.HomeView().render());
  },
  ...
  signUp: function() {
    ...
    $('.shell').html(app.signUpView.render());
    ...
    document.getElementById('passwordConfirm').addEventListener('change', function(event) {
  ...
  signIn: function() {
    ...
    $('.shell').html(app.signInView.render());
  ...
  eventList: function() {
    ...
    app.eventList = new app.EventsListView({
      model: app.Events
    }).render();
    $('.sideBar').html(app.eventList);
  ...
  eventDetails: function(id) {
    ...
    this.showView('.content', new app.EventView({
      model: event
    }));
  },
  ...
  newEvent: function() {
    ...
    app.eventView = new app.EventView({model: new app.Event()});
    $('.shell').html(new app.ShellView().render());
    this.showView('.content', app.eventView);
    this.eventList();

    document.getElementById('name').$.input.focus();
	// Dynamically Validate form as the user types
    // List of sign up input fields that will be validated
    this.$eventPaperCard = $('paper-card#event .form-control');
    // this.formValidation(this.$eventPaperCard);
